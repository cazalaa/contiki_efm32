...
extern uint8_t   *Radio_ConfigurationArray ;
...
uint8_t si446x_configuration_init(void)
{
  ...
  //uint8_t testjcc[] = RADIO_CONFIGURATION_DATA_ARRAY;
  uint8_t *pSetPropCmd = &Radio_Configuration_Data_Array[0] ;
  //uint8_t *pSetPropCmd = &testjcc[0];
  uint8_t col = 0;
  ...
  //GPIO_PinOutToggle(PORT_LED0, PORT_PIN_LED0);
  //bSpiReadWrite(0x5A);
  //bSpiReadWrite(testjcc[0]);
    //radio_hal_SpiWriteData(10,pSetPropCmd);
